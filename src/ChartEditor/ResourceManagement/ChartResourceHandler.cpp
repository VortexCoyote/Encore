	...
	for (auto i = 0; i < directory.size(); i++)
	{
		...
		while (std::getline(chartFile, line))
		{
			...
			if (line == "[Difficulty]")
			{
				...
				while (std::getline(chartFile, difficultyLine))
				{
					std::string type;
					std::string value;
					...
					if (type == "CircleSize")
						chartData->keyAmount = int(std::stof(value));
			...
			{
				std::string noteLine;
				while(std::getline(chartFile, noteLine))
				{
					...
					if (noteType == 128)
					{
						...
						note->column = CLAMP(floor(float(column) * (float(chartData->keyAmount) / 512.f)), 0, chartData->keyAmount - 1);
...
void ChartResourceHandler::SaveChart(std::string aFolderPath, std::string aPath, ChartData* aChart)
{
	...
	chartData << "CircleSize:" << std::to_string(chart->keyAmount) << std::endl;
	...
	{
		int column = float(float((note->column + 1)) * 512.f) / float(chart->keyAmount) - (512.f / float(chart->keyAmount) / 2.f);
